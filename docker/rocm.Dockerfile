...
# Dockerfile for AMD ROCm GPUs (x86).

ARG BASE_IMAGE=rocm/vllm-dev:latest
FROM ${BASE_IMAGE}
...
RUN --mount=type=cache,target=/root/.cache/uv \
    ...
    pip freeze > /tmp/excludes.txt && \
    uv pip install --system --break-system-packages --excludes /tmp/excludes.txt --extra rocm .
